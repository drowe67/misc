...
Consider a signal $x(t)$ consisting of a wanted signal at $\omega_1$, an adjacent signal at $\omega_2$, and an impulse train that repeats every $T=1/R$ seconds.
\begin{equation}
x(t) = a_1 e^{j \omega_1 t} + a_2 e^{j \omega_2 t} + a_3\sum_{n=-\infty}^{\infty}\delta(t-nT)
\end{equation}
To remove the effects of the impulse, a blanking pulse train $b(t)$ applied, each pulse having with $T$ seconds, and period $P$:
\begin{equation}
\begin{split}
b(t) &= 1 - p(t) \\
p(t) &= \sum_{n=-\infty}^{\infty}rect(t-nP) \\
rect(t) &= 
	\begin{cases}
      1, & |t| <= T/2 \\
      0, & otherwise \\
...
\end{split}
\end{equation}
where $\alpha=2 \pi R$ is the angular frequency of the pulse train, and $d$ is the duty cycle.  The Fourier transform of $b(t)$ is:
\begin{equation}
\begin{split}
B(w) &= \int_{-\infty}^{\infty}b(t)e^{-j \omega t}dt \\
     &= \int_{-\infty}^{\infty} e^{-j \omega t}dt - \int_{-\infty}^{\infty} d e^{-j \omega t}dt \\
     &- \int_{-\infty}^{\infty} d \sum_{n=1}^{\infty} sinc(\pi n d) e^{-j\alpha n}e^{-j \omega t}dt \\
     &- \int_{-\infty}^{\infty} d \sum_{n=1}^{\infty} sinc(\pi n d) e^{j\alpha n}e^{-j \omega t}dt \\
     &= (1-d) \delta(\omega) - d \sum_{n=1}^{\infty} sinc(\pi n d) \left[ \delta(\omega + \alpha n) + \delta(\omega - \alpha n) \right]
\end{split}
\end{equation}
