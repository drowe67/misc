...
Consider a signal $x(t)$ consisting of a wanted signal $s_1(t)$, an adjacent signal $s_2(t)$, and an unwanted impulse train that repeats every $1/R$ seconds.
\begin{equation}
\begin{split}
x(t) &= s_1(t) + s_2(t) + a_3\sum_{n=-\infty}^{\infty}\delta(t-n/R) \\
     &= a_1 e^{j \omega_1 t} + a_2 e^{j \omega_2 t} + a_3\sum_{n=-\infty}^{\infty}\delta(t-n/R)
\end{split}
\end{equation}
...
\begin{split}
y(t) &= x(t)b(t) \\
     &= a_1 e^{j \omega_1 t} + a_2 e^{j \omega_2 t} - a_1 e^{j \omega_1 t}p(t) - a_2 e^{j \omega_2 t}p(t) \\
     &= a_1 e^{j \omega_1 t} + a_2 e^{j \omega_2 t} - a_1 e^{j \omega_1 t}(d + dh(t))- a_2 e^{j \omega_2 t}(d + dh(t)) \\    
     &= a_1 (1-d) e^{j \omega_1 t} + a_2 (1-d) e^{j \omega_2 t} - a_1 d h(t) e^{j \omega_1 t} - a_2 d h(t) e^{j \omega_2 t} \\    
Y(w) &= a_1 (1-d) \delta(\omega - \omega_1) + a_2 (1-d) \delta(\omega - \omega_2) -a_1dH(\omega-\omega_1) - a_2dH(\omega-\omega_2) \\
     &= a_1 (1-d) \delta(\omega - \omega_1) + a_2 (1-d) \\
     &- a_1 d \sum_{n=1}^{\infty} sinc(\pi n d) \left[ \delta(\omega + \alpha n - \omega_1) + \delta(\omega - \alpha n - \omega_1) \right] \\
     &- a_2 d \sum_{n=1}^{\infty} sinc(\pi n d) \left[ \delta(\omega + \alpha n - \omega_2) + \delta(\omega - \alpha n - \omega_2) \right]
\end{split}
\end{equation} 

The fundamental terms have been reduced to $(1-d)$, and harmonic sidebands $H(\omega)$ has been introduced either side of $s_1$ and $s_2$.
Octave plots for $R>B$ and $R<B$
Exploring noise blanker
what can we say about digital modulation?
