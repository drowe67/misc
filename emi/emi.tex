...
\end{split}
\end{equation}
where $a_1$ and $a_2$ are complex constants that model gain and phase shift variations on the two antennas. It can be shown that:
...
E = r_1(t) - cr_2(t)
\end{equation}
This model implies the EMI signal must be present in both received signals. In diversity systems the two antennas are placed some fraction of a wavelength apart. Near field intensity is a strong function of distances of less than one wavelength, so the two antennas are likely to have very different responses to near field EMI. For example at a spacing of $\lambda/4$ a near field signal coupled to one antenna may be undetectable in the other antenna. This implies the EMI fields must be fully resolved far field signals for the diversity attack to work.  Alternatively antennas with small separation should be used such that near field signals are detected at adequate levels in both antennas.
...
Loop antennas (either tuned or untuned) have been claimed to help EMI in some circumstances, by virtue of lower sensitivity to near electric fields at certain distances from the loop.
...
The station antenna is an inverted V fan dipole with 40m and 20m elements, suspended over the house, the mast and feed point is 9m above ground. It is coupled to 50 ohm coax using a balun. Subjectively, SSB stations on the 40m band are hard to hear and unpleasant to listen to, and 20m varies between reasonable noise levels and completely unusable (time varying).

\begin{figure}[h]
\caption{Map of the authors home station.  EMI sources are power and xDSL lines, wiring in the home and adjacent homes, and roof mounted solar PV system}
\label{fig:home_map}
\begin{center}
...
A series of observations were made using a spectrum analyser
...
If the EMI is AWGN, which is the sum of many urban EMI signals. We cannot attack this with blanking, as it has no time domain pulse structure.  This leaves diversity.  

\subsection{Further Work}

Determine if my noise floor is near field or far field

There is much hearsay. Measurement of SNR, measure/detect common mode ccurrents, comparsion to theory.
next steps, core aim of testing theory that AWGN "urban sum" can be supressed using diversity coherent methods
