...
Consider the numerator of $|C_t|$ (\ref{eq:acq_corr}) when only noise with variance $N_0$ is present:
\begin{equation}
D_t = \bm{r}^H\bm{p} = \bm{n}^H\bm{p}
\end{equation}

For a typical sequence of pilots with equal amplitudes $Pc=\pm1$ the variance of $D_t$ will be the same as the variance of $\bm{n}$.  The magnitude of a complex Gaussian noise signal will have a Rayleigh distribution \cite{rayleigh} with CDF:
\begin{equation}
CDF(x) = 1 - e^{-x^2/(2 \sigma_r^2)}
\end{equation}
$CDF(x)$ is the probability of any sample of the random variable being less than $x$.  The probability of any sample exceeding $D_t$ is therefore $1-CDF(|D_t|)$:
\begin{equation}
...
We can therefore set a threshold probability and solve for $|D_{thresh}|$:
\begin{equation}
|D_{thresh}| = N_0 \sqrt{-ln(P_{thresh})}
\end{equation}
When a signal is present the distribution will no longer be Rayleigh.  In fact the variance of $D_t$ will be quite large due to partial correlation matches with payload data symbols. However in this case $|D_{tmax}|$ will be considerably larger due to strong correlation peaks with $p(n)$, and will comfortably exceed any threshold calculated from noise only.

The new acquisition algorithm is therefore:
\begin{enumerate}
\item Set a threshold $P_{thresh}$ small enough so that $D_{tmax}>D_{thresh}$ is unlikely on noise only.
\item Estimate the noise power $N_0$ over a window of one modem frame $T_f$ seconds.
\item Find the peak $D_{tmax}$ over the window of $T_f$ seconds and calculate the $P_a$ using (\ref{eq:prob_thresh}).
\item If $P_a<P_{thresh}$ we have found a candidate modem frame.
\end{enumerate}
