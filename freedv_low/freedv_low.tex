...
When a signal is present the distribution will no longer be Rayleigh.  In fact the variance of $|D_t|$ will be somewhat larger than the noise alone case due to partial correlation matches with payload data symbols. However $|D_t|$ exhibits strong correlation peaks with $p(n)$, so comfortably exceeds any threshold calculated from the noise only model.
...
\item At run time, estimate the noise power $\sigma^2=Var(r(n))$ over a window of one modem frame $T_f$ seconds. This will be an approximation, as $\bm{r}$ includes noise and wanted signal.
\item Find the peak $D_{tmax}$ over the window of $T_f$ seconds.
\item Calculate $D_{thresh}$ using (\ref{eq:corr_prob}).  Note that unlike the existing 700D/E acquisition algorithm $D_{thresh}$ is dynamic, and based on current the variance of the received signal.
