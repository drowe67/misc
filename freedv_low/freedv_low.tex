...
TODO: Estimate MIP with +/- 200 Hz frequency offset estimation. Compare to 700D.  Compare to load when demodulating.  How vectorised can we get it, e.g. organising as a big matrix multiply might vectorise better. Maths/example for using FFT, issue in that it calculates every step.

\subsection{Frequency Offset Refinement and Tracking}

\begin{figure}[H]
\caption{Histograms of fine freq error using a $0.01R_s$ grid to evaluate $D_{tmax}$. $E_b/N_0=100$ dB is the ideal, no noise case; the other two cases are worst case, threshold $E_b/N_0$ for the channels.}
\label{fig:acq_fine_hist}
\begin{center}
\input acq_fine_hist.tex
\end{center}
\end{figure}

TODO: Should we do fine freq sampling/refinement before UW?  I think so, as it will improve UW results.  What is req for timing/freq error for successful acquisition?  I guess within capability of phase correction in equalisation to resolve, x\% of the time.

For tracking we wish to be able to follow a certain rate of change, and resolve freq error to within range of phase est to resolve.
...
\item Consider an "end of over frame", signal a clean end of over and squelch.
\item test with impulse noise and speech
\end{enumerate}
...
Idea: extra carrier(s), that supply a -ve pulse only when required, so they would be zero energy most of the time.  Just one symbol long.  Must be the shape of one cycle, to stay orthogonal to other carriers.  Can we describe this with math?  What benefit will it give?

PAPR defined by probability - we don't mind clipping every 5 seconds.
