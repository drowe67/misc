...
Maintaining a viable digital voice link on multipath channels is challenging, as there are periods when the SNR of a packet is very low across all carriers. For HF data we can use long FEC codewords to ride over the fades but latency concerns make this difficult for PTT speech.
...
\caption{BER/packet for 0.16s packets, same channel as Figure \ref{fig:spectrogram}.  Larger packets allow us to spread the errors over time, beneath a BER threshold where they can be corrected by FEC.}
...
TODO: spectrogram figure, discuss why time domain diversity is required.
TODO: BER per packet figure, I think this is instructive, explain swhy smoothing out errors in time will help.
TODO: smaller BER/PER plot so they don't all end up one per page?

This section WIP, currently in note form. For the $n=2$ diversity simulation (Figure \ref{fig:div_ber_curves}), the signal was split into 2 copies, with half the power going to each copy.  The 2nd copy was shifted 1000 Hz higher in frequency.
...
\item TODO: simulate low rate code.  This makes sense giving $H_c$ are correlated, ie entire channel is often quite down.
\end{enumerate}


\begin{figure}[h]
\caption{$n=2$ diversity simulation BER results. The 1.8 and 3.6 second curves simulate long delay $D$ (long FEC codewords). Note there is no effect on BER with large codewords as these curves are the average BER over the entire simulation, they do not model the non-uniform distribution of errors on multipath channels.}
