...
The two path HF channel model is given by:
\begin{equation}
y(n) = x(n)e^{j\phi_1}+x(n-dF_s)e^{j\phi_2}
\end{equation}
where $\phi_1$ and $\phi_2$ are two time varying, complex, Gaussian filtered random variables with bandwidth $F_d$ Hz, $d$ is the path delay in seconds and $F_s$ is the sample rate in $\si{Hz}$. As $F_d << R_s$ we assume $\phi_1$ and $\phi_2$ are stationary for the duration of a single symbol. Taking the z-transform:
\begin{equation}
\begin{split}
Y(z) &= X(z)e^{j\phi_1}+X(z)z^{-dF_s}e^{j\phi_2} \\
\frac{Y(z)}{X(z)} &= e^{j\phi_1}+z^{-dF_s}e^{j\phi_2} \\
H(z) &= e^{j\phi_1}+z^{-dF_s}e^{j\phi_2} \\
H(e^{j \omega}) &= e^{j\phi_1}+e^{j(\phi_2 - \omega d F_s)}
\end{split}
\end{equation}
...
\begin{equation}
H_n = e^{j\phi_1}+e^{j (\phi_2 - 2 \pi R_s d)}
\end{equation} 
We note there is some symmetry in the RH term in the phase between carrier $n-1$ and carrier $n+1$ which supports averaging over adjacent carriers to estimate $H_n$.
