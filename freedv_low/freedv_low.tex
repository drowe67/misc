...
The correlation algorithm can be derived from a least squared error formation.  Consider the total squared error between the time domain pilot signal $p(n)$ and the ideal received pilot sequence $p^\prime(n)$:
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
where $r(n)$ is the received off air samples at the current time offset $N_{t}$, and $g$ is a complex scalar gain that models level and phase variations.  Expressing $p(n)$ and $r(n+N_t))$ as column vectors:
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
where $C_t$ is the correlation between the received signal at time $N_t$ (in samples) and the known transmitted pilot signal. Minimising E is equivalent to maximising the correlation $C_t$ or equivalently $\sqrt{C_t}$. Typically $C_t$ is sampled on a grid of $N_t$ as $r(n)$ are received and $N_t$ advances.  When $C_t$ exceeds a threshold, we consider the current $N_t$ as a candidate coarse time offset for further processing.

A common acquisition strategy is to add one or more non-data pilot carriers to aid frequency offset estimation. These carriers consume power and carry no payload data so increase the threshold SNR required for a voice link. Consider the effect of $E_b$ when adding an additional carrier:
\begin{equation}
\begin{split}
E_b &= \frac{ST_s}{N_c} \\
...
\end{split}
\end{equation}
where $L_{pc}$ is the link degradation in dB for adding a single pilot carrier.  For example given a 16 carrier waveform, $L_{pc}=10log_{10}(17/16)=0.26 \si{dB}$, which is quite modest for the potential gains in acquisition performance.  A pilot carrier is also likely to show up visibly on a real time waterfall and provide a useful manual tuning aid.
...
\item There may be a good way to select an appropriate sequence for pilots to maximise the probability of detection.
