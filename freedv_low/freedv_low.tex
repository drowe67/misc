...
When a signal is present the distribution of $|D_t|$ will no longer be Rayleigh due to the correlation peaks with $p(n)$. Figure \ref{fig:acq_dt_hist} is a histogram of $|D_t|$, the RHS hump in the histogram is due to the correlation peaks.  This is even more obvious in the scatter plot of $D_t$ (Figure \ref{fig:acq_dt_scatter}). It was found that by removing the outliers in a set of $D_t$ samples and recalculating the standard deviation, a good fit can be found with the Rayleigh distribution (green in Figure \ref{fig:acq_dt_hist}).  The outlier removal algorithm is:
\begin{equation}
\label{eq:prob_thresh}
\begin{split}
\sigma &= Std(D_t) \\
D^\prime_t &= D_t < 2\sigma \\
\sigma^\prime &= Std(D^\prime_t)
\end{split}
\end{equation}
Any outlier greater than $2\sigma$ is removed from the set used to calculate the standard deviation, and $\sigma^\prime$ is used to calculate $D_{thresh}$. Note the variance of $D_t$ with signal present is larger than the variance with noise alone, due to random correlations of the modem signal with $p(n)$. The circle in Figure \ref{fig:acq_dt_scatter} shows the threshold calculated for this example.

\begin{figure}[H]
\caption{Histogram of $D_t$ sampled once a symbol over 450 modem frames, for $E_b/N_0=0$ dB.  After removing the outliers due to strong correlation with $p(n)$ on the RHS, the LHS of the histogram is a good fit to the Rayleigh PDF.}
...
\begin{figure}[H]
\caption{Scatter plot of $D_t$ on the complex plane from the same simulation as Figure \ref{fig:acq_dt_hist}. The circle is $D_{thresh}$ from (\ref{eq:corr_prob}) using $\sigma^\prime$ and $P_{thresh}=10^{-3}$.}
...
\item Calculate a set of $D_t$ values over the window.
\item Find the peak $D_{tmax}$ over the set of $D_t$ samples.
\item Estimate the standard deviation $\sigma$, then remove outliers to obtain $\sigma^\prime$.
...
D_t = g N_c M
\end{equation}
For the case where $\omega \ne 0$, analysis is more difficult.  However we can observe that the numerator $1 - e^{j 2 \pi q}$ will pass through local maxima and minima at multiples of $\omega = 2 \pi / M$, i.e. multiples of one carrier spacing $R_s$. As $|\omega|$ increases, $q \ne 0$ when $d-c=0$, so the denominator $1 - e^{j 2 \pi q}$ will be larger than 0 in the limiting case, gradually reducing the peak of the local maxima.  

Thus $D_t$ will have a global maxima at $\omega=0$, and some local minima and maxima separated by $Rs$. The local maxima makes an iterative solution based on gradient descent problematic.
Instead, consider adding a frequency correction term $\hat{\omega}$ to $r(n)$ from (\ref{eq:dt_freq}):
\begin{equation}
\begin{split}
\label{eq:corr_freq}
...
\end{split}
\end{equation}
When $\hat{\omega} = \omega$, the two frequency shift terms will cancel out and we will be left with the ideal  case, maximising $\hat{D}_t$. The frequency offset can therefore be found by sampling (\ref{eq:corr_freq}) on a sufficiently fine grid of $\hat{\omega}$.  
