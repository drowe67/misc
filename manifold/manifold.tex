...
\begin{figure}[H]
\caption{ML Experiment Block Diagram. The loss function $L(\mathbf{y},\hat{\mathbf{y}})$ only used during training. The algorithm outputs the rate $L$ harmonic amplitudes $\mathbf{a}$ and phases $arg[H(e^{j\omega_0 m})]$ for sinusoidal synthesis \cite{rowe2023_codec2}.  It can be viewed as an autoencoder with the bottleneck being the smoothed rate $K=20$ vector $\mathbf{b}$.}
...
\node [block, below of=est,node distance=2cm,text width=2cm] (resample_k80) {Resample Rate $K=80$};
\node [block, below of=dft,node distance=2cm,text width=2cm] (filter) {Filter $N_b=20$};
\node [block, below of=filter,node distance=2cm,text width=2cm] (resample_k20) {Resample Rate $K=20$};
\node [block, below of=resample_k80,node distance=2cm] (nn) {$F(\mathbf{b},\omega_0,v)$};
\node [block, below of=nn,node distance=2cm,text width=2cm] (resample_l) {Resample Rate $L$};
\node [block, below of=resample_k20,node distance=2cm,text width=2cm] (hilbert) {Hilbert Transform};
...
\node [output, below of=resample_l,node distance=2cm] (output1) {};
\node [output, below of=hilbert,node distance=2cm] (output2) {};
...
\draw [->] (z) |- (nlp);
\draw [->] (nlp) --  node[text width=2cm] {$\omega_0$}(output3);
\draw [->] (window) -- node[below] {$s_w(n)$} (dft);
\draw [->] (dft) -- node[below] {$S_\omega(k)$} (est);
\draw [->] (est) -- node[left] {$\mathbf{a}$} (resample_k80);
\draw [->] (est) -- node[above] {$\mathbf{a}$} (voicing);
\draw [->] (voicing) -- node[left] {$v$} (output4);
\draw [->] (resample_k80) -- node[above] {$\mathbf{y}$} (filter);
\draw [->] (filter) -- (resample_k20);
\draw [->] (resample_k20) -- node[above] {$\mathbf{b}$} (nn);
\draw [->] (resample_k80) -| node[above] {$\mathbf{y}$} (loss);
\draw [->] (nn) -- node[left] {$\hat{\mathbf{y}}$} (resample_l);
\draw [->] (nn) |- (z1) -- (loss);
\draw [->] (resample_l) -- node[above] {$\hat{\mathbf{a}}$} (hilbert);
\draw [dotted] (loss) -- (nn);
\draw [->] (resample_l) -- node[left] {$\hat{\mathbf{a}}$} (output1);
