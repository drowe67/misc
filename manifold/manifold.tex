...
Figure \ref{fig:ml_experiment} is a block diagram of the ML experiment signal processing. The loss function $L(\mathbf{y},\hat{\mathbf{y}})$ only used during training. The algorithm outputs the rate $L$ harmonic amplitudes $\mathbf{a}$ and phases $arg[H(e^{j\omega_0 m})]$ for sinusoidal synthesis \cite{rowe2023_codec2}.  Pre-emphasis and de-emphasis is applied in the frequency domain. The ML network $F$ and loss function $L$ operates on normalised energy vectors as we are interested in matching spectral shape, not gain (see also manifold.py comments). It can be viewed as an autoencoder with the bottleneck being the smoothed rate $K=20$ vector $\mathbf{b}$.
 
\begin{figure}[H]
\caption{ML Experiment Block Diagram.}
...
\node [block, right of=dft,node distance=3cm,text width=1.5cm] (est) {Est Rate $L$ Amp};
...
\node [block, below of=pre,node distance=1.5cm,text width=2cm] (resample_k80) {Resample Rate $K=80$};

\node [circ,  right of=resample_k80,node distance=3cm] (sub1) {$-$};
\node [input, above of=sub1, node distance=1cm] (y_bar1) {};

\node [block, below of=resample_k80,node distance=1.5cm] (nn) {$F(\mathbf{b},\omega_0,v)$};

\node [circ,  left of=nn,node distance=2cm] (sub2) {$-$};
\node [input, above of=sub2, node distance=0.75cm] (y_bar3) {};

\node [block, left of=sub2,node distance=2.5cm,text width=2cm] (resample_k20) {Resample Rate $K=20$};
...
\node [block, left of=add1,node distance=3cm,text width=2cm] (resample_l) {Resample Rate $L$};
...
\draw [->] (resample_l) -- (de);
\draw [->] (nn) |- (z1) -- (loss);
\draw [->] (de) -- node[above] {$\hat{\mathbf{a}}$} (hilbert);
\draw [dotted] (loss) -- (nn);
