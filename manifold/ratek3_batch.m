...
function B = ratek3_batch_tool(samname, varargin)
  ...
  for f=1:frames
    ...
    if rateK_en
      ...
      % back to rate L
      AmdB_ = interp1([0 rate_Lhigh_sample_freqs_kHz 4], [0 YdB(f,:) 0], rate_L_sample_freqs_kHz, "spline", "extrap");
      if norm_en
        AmdB_ = norm_energy(YdB(f,:), AmdB_);
      end
      if pre_en
        AmdB_ += PdB;
      end
      ...
      if length(H_out_fn)
        H(f,1:L) = synth_phase_from_mag(rate_Lhigh_sample_freqs_kHz, YdB(f,:), Fs, Wo, L, phase_pf_en);
      end
    end
...
% Returns a file of b vectors, similar to ratek80_batch_tool() above but all options stripped out, used for input
% to ML inference tool
function ratek80_batch_ml_in(samname, varargin)
  more off;
  ...
  % write b to a .f32 file for external VQ training
...
function ratek80_batch_ml_out(samname, varargin)
  more off;
  ...
  printf("Y_in_fn: %s A_out_fn: %s H_out_fn: %s\n", Y_in_fn, A_out_fn, H_out_fn);
